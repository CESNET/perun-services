...
class FileLock:
    ...
    def __init__(self, lockfile):
        ...
        try:
            ...
            die_with_error(
                "Unable to get lock, service propagation was already running."
            )
...
def copy_files_to_directory(
    path_from: str, path_to: str, name_pattern: re.Pattern = None
) -> None:
    """
...
def create_lock(
    service_name: str, destination: str, custom_lock_dir: str = None
) -> FileLock:
    """
    ...
    if destination is not None:
        lockfile = f"perun-{service_name}-{escape_filename(destination)}.lock"
    else:
        lockfile = f"perun-{service_name}.lock"
