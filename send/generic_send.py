...
if __name__ == "__main__":
    ...
    destination_type = destination_type_host if len(sys.argv) == 3 else sys.argv[3]
    ...
    with tempfile.NamedTemporaryFile(mode="w+") as temp:
        if destination_type == destination_type_url:
            ...
            if (
                os.access(perun_cert, os.R_OK)
                and os.access(perun_key, os.R_OK)
                and os.access(perun_chain, os.R_OK)
            ):
                transport_command.extend(
                    ["--cert", perun_cert, "--key", perun_key, "--cacert", perun_chain]
                )
        ...
        if destination_type == destination_type_host:
            ...
            if locals().get("windows_proxy") is None:
                ...
                if windows_proxy is None:
                    print(
                        "Variable WINDOWS_PROXY is not defined. "
                        "It is usually defined in /etc/perun/services/generic_send/generic_send.conf.",
                        file=sys.stderr,
                    )
                    exit(1)
            if not (re.fullmatch(userAtHostPattern, destination)):
        ...
        with tempfile.NamedTemporaryFile(
            mode="w+", prefix="hostname_", dir=temp_dir.name
        ) as temp_hostname_file:
            temp_hostname_file.write(hostname)
            temp_hostname_file.flush()
            temp_file_name = os.path.basename(temp_hostname_file.name)
            ...
            if destination_type == destination_type_user_host_windows_proxy:
                ...
                transformation_process = subprocess.Popen(
                    sed_command,
                    stdin=transformation_process_1.stdout,
