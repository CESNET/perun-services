...
### Example command - contains import token filename (t) and directory (d) with json objects to be imported.
### File with import token has to contain only the import token.
...
# How many times should the execution status be called for result stats
my $maxwait = 10;
#--------------------------------------------------------------------------
...
Token file contains only import token.
...
Mandatory parameters:
 --tokenFilename       | -t file with import token
...
GetOptions("help|h"	=> sub {
	...
	"tokenFilename|t=s" => \$tokenFilename,
	"importDirectory|d=s" => \$importDirectory) || die help;

unless ($tokenFilename) {die "Token filename (-t) is required!\n"}
unless ($importDirectory) {die "Import directory (-d) is required!\n"}

# Check token file exists and contains token
open FILE, $tokenFilename or die "Could not open config file $tokenFilename: $!\n";
my $token = <FILE>;
chomp $token;
close FILE or die "Could not close config file $tokenFilename: $!\n";
unless ($token) {die "Token is missing in token file!\n"}

# Call INFO endpoint with token to obtain endpoints
my $info_links = callServer($INFO_URL, $TYPE_GET);
unless ($info_links->{'links'}) {die "Missing data in server response!\n"}
...
unless ($import_links->{'links'}) {die "Missing data in server response!\n"}
...
### Now also progress endpoint can be called, which returns Insight's info about processing the import
### Basic auth is used, token is base64 encoded '<mail>:<API token>' (requires generated API token and privileges)
### https://api.atlassian.com/jsm/insight/workspace/<workplace id>/v1/progress/category/imports/<import id>
...
sub importFilesFromDirectory {
	...
	while ($filename = readdir(DIR)) {
		...
		my $data = do { local $/; <$fh> };
		close $fh or die "Unable to close file $filepath $!\n";
