...
def send(
    ...
    with tempfile.NamedTemporaryFile(mode="w+") as temp:
        if (
            transport_command is None
            and destination_type == send_lib.DESTINATION_TYPE_URL
        ):
        ...
        host, hostname, port = send_lib.prepare_destination(
        ...
        with (
            send_lib.prepare_temporary_directory() as hostname_dir,
            ...
            # Save copy of files to persisted directory for auditing
            send_lib.persist_spool_files(
                facility_name,
                service_name,
