...
function process {
	...
	FROM_PERUN_DIR="${WORK_DIR}/sshkeys_generic/"
	PASSWD="/etc/passwd"	
	...
	do
		...
		if [ ! -d $DST_HOME_DIR/.ssh/ ]; then
			mkdir $DST_HOME_DIR/.ssh/
			chown $USER_NAME:$GID $DST_HOME_DIR/.ssh/
			chmod 0700 $DST_HOME_DIR/.ssh/
		fi

		# If .k5login not exists, create new one, in other case use diff on them
		DST_FILE="$DST_HOME_DIR/.ssh/authorized_keys"
		if [ ! -f "$DST_FILE" ]; then
