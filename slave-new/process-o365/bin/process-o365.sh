...
function process {
	...
	# GET licenses:
	curl --header "Authorization: $ACCESS_TOKEN" --header "Accept: application/json" 'https://graph.microsoft.com/v1.0/subscribedSkus' | perl -nle 'print "$1,$2" while(/"skuId"\s*:\s*"([^"]*)","skuPartNumber"\s*:\s*"([^"]*)"/g)' >> $HOME_DIR/$LICENSES

	PERL_SYNCHRONIZE_USERS_SCRIPT='
		use LWP::UserAgent;
		use MIME::Base64;
		use JSON;
		...
		my $o365UserMapper = {};
		my $filenameLicenses = '${HOME_DIR}/$LICENSES';
		...
		# parse licenses
		open FILE_LICENSES, "<", "$filenameLicenses" or die "Cant open '$filenameLicenses': $!";
		while (my $lineLicenses = <FILE_LICENSES>) {
			chomp $lineLicenses;
			@licenseDetails = split(/,/, $lineLicenses);
			$licenses->{$licenseDetails[1]} = $licenseDetails[0];
		}
		close FILE_LICENSES;

		my $ua = LWP::UserAgent->new;
		my $server_endpoint = "https://graph.microsoft.com/v1.0/users";
		...
		# obtain users from o365 and load them into listOfUsersActual structure
	...
	catch_error E_GROUP_MERGE perl -e "${PERL_SYNCHRONIZE_USERS_SCRIPT}"

	# delete all files for API purpose only
	if [-f ${HOME_DIR}/$LICENSES]
	then
		rm ${HOME_DIR}/$LICENSES
	fi
}
